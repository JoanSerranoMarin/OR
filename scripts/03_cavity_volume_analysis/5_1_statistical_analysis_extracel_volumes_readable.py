...
# Font sizes tuned for a compact full-width two-column figure
FONT_SIZE = 6.5
LABEL_SIZE = 7.0
TICK_SIZE = 5.5
PANEL_SIZE = 8.0
...
def main():
    ...
    ap.add_argument(
        "--outfig",
        default="figure2_extracellular_cavities.png",
        help="Output raster image file, usually PNG or TIFF.",
    )
    ap.add_argument(
        "--outpdf",
        default="figure2_extracellular_cavities.pdf",
    ...
    ap.add_argument(
        ...
        type=float,
        default=7.2,
        help="Figure width in inches. 7.2 inches is close to full two-column width.",
    )
    ap.add_argument(
        "--fig-height",
        type=float,
    ...
    if imC is not None:
        cbC = fig.colorbar(imC, ax=axC, fraction=0.046, pad=0.02)
        cbC.set_label(r"|Δmean log$_2$FC|", fontsize=LABEL_SIZE)
        cbC.ax.tick_params(labelsize=TICK_SIZE, length=1.5, width=0.4)
    ...
    if imF is not None:
        ...
        cbF.set_label(r"|Δmean log$_2$FC|", fontsize=LABEL_SIZE)
        cbF.ax.tick_params(labelsize=TICK_SIZE, length=1.5, width=0.4)
    ...
    if imI is not None:
        cbI = fig.colorbar(imI, ax=axI, fraction=0.046, pad=0.02)
        cbI.set_label(r"|Δmean log$_2$FC|", fontsize=LABEL_SIZE)
        cbI.ax.tick_params(labelsize=TICK_SIZE, length=1.5, width=0.4)
    ...
    fig.legend(handles=handles, loc="upper center", ncol=2,
               frameon=False, bbox_to_anchor=(0.50, 1.015),
