...
across GPCR classes, combining three ROIs into a compact 3×2 panel figure.

This version removes the right-column heatmaps and encodes post-hoc
between-class comparisons directly above the log2FC panels using a compact
letter display (CLD): classes that share at least one letter are not
significantly different; classes that share no letters are significantly
different according to the selected post-hoc test.

Expected input Excel files:
- volumen_5_portal_EC/gpcr_log2_active_inactive.xlsx
- volumen_2_ec_vestibule/gpcr_log2_active_inactive.xlsx
- volumen_3_cleft_volume/gpcr_log2_active_inactive.xlsx

Outputs:
- figure2_extracellular_cavities_3x2_letters.png
- figure2_extracellular_cavities_3x2_letters.pdf
- gpcr_extracellular_stats_BH_3x2_letters.xlsx
"""
...
# ---------------------------------------------------------------------
# Plot style: designed for a full-width figure in a two-column document
# ---------------------------------------------------------------------
...
def class_display_name(name: str) -> str:
    """Return short labels: Class A -> A, Class B1 -> B1, etc."""
    s = str(name).strip()
...
def compact_letter_display(summary: pd.DataFrame, pairs_df: pd.DataFrame, alpha=0.05):
    ...
    # Sort by mean, as normally done for compact letter displays.
    sorted_classes = sorted(classes, key=lambda c: means.get(c, np.nan), reverse=True)

    letters = []  # list of sets of classes
    letter_names = []
    ...
    # Second pass: add compatible letters to make the display more compact.
    changed = True
    while changed:
        changed = False
        for c in sorted_classes:
...
def plot_active_inactive_absolute_ax(ax, df: pd.DataFrame, classes_order,
    ...
    # Leave room at the top for active/inactive significance lines.
...
def plot_log2_fc_letters_ax(ax, summary: pd.DataFrame, df: pd.DataFrame,
    ...
    # Compact post-hoc letters at the top of the panel.
    for i, cls in enumerate(classes):
    ...
    # Optional fold-change labels; useful but kept small to avoid dominating the figure.
    if show_fold_labels:
        off = 0.045 * yr
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate compact 3×2 multi-ROI figure with post-hoc letters instead of heatmaps."
    )
    ap.add_argument("--portal-xlsx", default="volumen_5_portal_EC/gpcr_log2_active_inactive.xlsx",
                    help="Path to portal ROI Excel.")
    ap.add_argument("--ecv-xlsx", default="volumen_2_ec_vestibule/gpcr_log2_active_inactive.xlsx",
                    help="Path to EC vestibule ROI Excel.")
    ap.add_argument("--cleft-xlsx", default="volumen_3_cleft_volume/gpcr_log2_active_inactive.xlsx",
                    help="Path to TM5–TM7 cleft ROI Excel.")
    ap.add_argument("--outfig", default="figure2_extracellular_cavities_3x2_letters.png",
                    help="Output PNG file.")
    ap.add_argument("--outpdf", default="figure2_extracellular_cavities_3x2_letters.pdf",
                    help="Output PDF file.")
    ap.add_argument("--excel-out", default="gpcr_extracellular_stats_BH_3x2_letters.xlsx",
                    help="Output Excel file with statistics and compact letters.")
    ap.add_argument("--alpha", type=float, default=0.05,
                    help="Significance threshold.")
    ap.add_argument("--fig-width", type=float, default=7.25,
                    help="Figure width in inches; 7.0–7.4 is typical for full-width two-column figures.")
    ap.add_argument("--fig-height", type=float, default=8.0,
                    help="Figure height in inches.")
    ap.add_argument("--no-fold-labels", action="store_true",
                    help="Do not print fold-change labels on log2FC panels.")
    args = ap.parse_args()

    set_style()

    portal = process_dataset(Path(args.portal_xlsx), args.alpha, roi_label="Portal")
    ecv = process_dataset(Path(args.ecv_xlsx), args.alpha, roi_label="EC vestibule")
    cleft = process_dataset(Path(args.cleft_xlsx), args.alpha, roi_label="TM5–TM7 cleft")

    datasets = [portal, ecv, cleft]
    roi_titles = ["Portal", "EC vestibule", "TM5–TM7 cleft"]
    panel_labels = [("A", "B"), ("C", "D"), ("E", "F")]

    fig, axes = plt.subplots(
        3, 2,
        figsize=(args.fig_width, args.fig_height),
        gridspec_kw={"width_ratios": [1.12, 1.0], "wspace": 0.24, "hspace": 0.46},
    )

    for row, (data, roi_name, labels) in enumerate(zip(datasets, roi_titles, panel_labels)):
        df, summary, pairs, classes_order, state, class_to_letters, letters_df = data
        ax_abs, ax_log = axes[row]

        plot_active_inactive_absolute_ax(ax_abs, df, classes_order, state, show_ylabel=True)
        plot_log2_fc_letters_ax(ax_log, summary, df, class_to_letters,
                                show_ylabel=True,
                                show_fold_labels=not args.no_fold_labels)

        ax_abs.set_title(f"{roi_name}: volume", fontsize=LABEL_SIZE, pad=5)
        ax_log.set_title(f"{roi_name}: activation effect", fontsize=LABEL_SIZE, pad=5)

        add_panel_label(ax_abs, labels[0])
        add_panel_label(ax_log, labels[1])

    # Shared legend, kept outside plotting area.
    handles = [
    ...
    fig.legend(handles=handles, loc="upper center", ncol=2, frameon=False,
               bbox_to_anchor=(0.52, 1.015), handlelength=1.2, columnspacing=1.3)

    # A short note in the margin explaining the letter display.
    fig.text(
        0.99, 0.01,
    ...
    fig.savefig(args.outfig, dpi=600)
    ...
    # Excel with full statistics and compact letter display.
    state_all = pd.concat([d[4] for d in datasets], ignore_index=True)
    class_all = pd.concat([d[2] for d in datasets], ignore_index=True)
    summary_all = pd.concat([d[1] for d in datasets], ignore_index=True)
    letters_all = pd.concat([d[6] for d in datasets], ignore_index=True)

    with pd.ExcelWriter(args.excel_out) as writer:
        state_all.to_excel(writer, sheet_name="state_active_vs_inactive_BH", index=False)
        class_all.to_excel(writer, sheet_name="log2FC_between_classes_posthoc", index=False)
        summary_all.to_excel(writer, sheet_name="summary_by_class", index=False)
        letters_all.to_excel(writer, sheet_name="posthoc_letters", index=False)
