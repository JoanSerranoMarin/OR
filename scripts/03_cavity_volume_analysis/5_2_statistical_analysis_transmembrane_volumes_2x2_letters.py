...
"""
Compact two-ROI GPCR volume figure.
...
labels above the log2FC panels. The resulting layout is 2×2:
    ...
    ROI 1 absolute volume      ROI 1 log2(active/inactive) + post-hoc letters
    ROI 2 absolute volume      ROI 2 log2(active/inactive) + post-hoc letters
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate compact 2×2 two-ROI GPCR volume figure with post-hoc letters instead of heatmaps."
    )
    ap.add_argument(
        "--roi1-xlsx",
        default="volumen_5_portal_EC/gpcr_log2_active_inactive.xlsx",
        help="Path to ROI 1 Excel.",
    )
    ap.add_argument(
        "--roi1-label",
        default="ROI 1",
        help="Label for ROI 1, used in plot titles and Excel output.",
    )
    ap.add_argument(
        "--roi2-xlsx",
        default="volumen_2_ec_vestibule/gpcr_log2_active_inactive.xlsx",
        help="Path to ROI 2 Excel.",
    )
    ap.add_argument(
        "--roi2-label",
        default="ROI 2",
        help="Label for ROI 2, used in plot titles and Excel output.",
    )
    ap.add_argument(
        "--outfig",
        default="figure_twoROIs_2x2_letters.png",
        help="Output PNG file.",
    )
    ap.add_argument(
        "--outpdf",
        default="figure_twoROIs_2x2_letters.pdf",
        help="Output PDF vector file.",
    )
    ap.add_argument(
        "--excel-out",
        default="gpcr_twoROIs_stats_BH_2x2_letters.xlsx",
    ...
    roi1 = process_dataset(Path(args.roi1_xlsx), args.alpha, roi_label=args.roi1_label)
    roi2 = process_dataset(Path(args.roi2_xlsx), args.alpha, roi_label=args.roi2_label)

    datasets = [roi1, roi2]
    roi_titles = [args.roi1_label, args.roi2_label]
