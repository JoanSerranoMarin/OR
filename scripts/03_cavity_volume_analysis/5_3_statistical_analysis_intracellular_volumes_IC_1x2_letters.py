...
def main():
    ap = argparse.ArgumentParser(
        description="Generate compact 1×2 single-region GPCR volume figure with post-hoc letters instead of a heatmap."
    )
    ap.add_argument(
        "--xlsx",
    ...
    ap.add_argument(
        "--outfig",
        default="figure_intracellular_cavity_1x2_letters.png",
        help="Output PNG file.",
    )
    ap.add_argument(
        "--outpdf",
        default="figure_intracellular_cavity_1x2_letters.pdf",
        help="Output PDF vector file.",
    )
    ap.add_argument(
        "--excel-out",
        default="gpcr_intracellular_cavity_stats_BH_1x2_letters.xlsx",
    ...
    print(f"[INFO] Region label inferred for input: {roi_label}")
    ...
    fig, axes = plt.subplots(
        ...
        gridspec_kw={"width_ratios": [1.12, 1.0], "wspace": 0.25},
    )
    ax_abs, ax_log = axes
    ...
    ax_abs.set_title(f"{roi_label}: volume", fontsize=LABEL_SIZE, pad=5)
