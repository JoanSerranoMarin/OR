...
# For log2 panels we use the "active" brown
...
TEXT_SIZE_FC = max(6, TEXT_SIZE - 2)  # slightly smaller for ×fold labels
...
def plot_active_inactive_absolute_ax(ax, df: pd.DataFrame, classes_order,
    ...
    Uses pre-computed paired t-tests (state_tests_df) to draw
    BH-corrected significance lines above each pair of bars.
    """
    ...
    # -----------------------------------------------------------------
    # Draw significance lines from precomputed BH-corrected tests
    # -----------------------------------------------------------------
...
def plot_means_transparent_ax(ax, summary: pd.DataFrame, df: pd.DataFrame):
    ...
    Uses same brown palette as active state; fold-change labels slightly smaller.
    """
    ...
    for i, mu in enumerate(means):
        ...
        ax.text(
            x[i], y_txt, f"×{fold:.2f}",
            ha="center", va="bottom",
            fontsize=TEXT_SIZE_FC,  # smaller only here
...
def main():
    ...
    axG.text(-0.1, 1.05, "G)", transform=axG.transAxes,
             fontsize=TEXT_SIZE+4, fontweight="bold")
    axH.text(-0.1, 1.05, "H)", transform=axH.transAxes,
             fontsize=TEXT_SIZE+4, fontweight="bold")
    axI.text(-0.1, 1.05, "I)", transform=axI.transAxes,
             fontsize=TEXT_SIZE+4, fontweight="bold")

    if imI is not None:
        cbI = fig.colorbar(imI, ax=axI, fraction=0.046, pad=0.04)
        cbI.set_label(r"|meanᵢ − meanⱼ| (Å³)")
