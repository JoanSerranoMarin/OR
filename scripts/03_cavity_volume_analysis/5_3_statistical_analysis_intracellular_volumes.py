...
"""
Single-ROI figure: extracellular cavity volumes and activation-dependent changes
across GPCR classes, for ONE gpcr_log2_active_inactive.xlsx file.

Outputs:
- A 1×3 figure (A–C) with:
  A) Absolute volumes (Å³): Inactive vs Active per class + paired t-test (BH) lines.
  B) log2(Active/Inactive) per class + points + fold-change labels (×1.23, etc.).
  C) Between-class contrasts in log2(Active/Inactive) (|Δmean| matrix + stars).

- An Excel file with:
  * state_active_vs_inactive_BH: paired tests Active vs Inactive per class.
  * log2FC_between_classes_posthoc: post-hoc class comparisons between classes.
  * summary_by_class: mean log2(Active/Inactive) per class.
"""
...
def plot_active_inactive_absolute_ax(ax, df: pd.DataFrame, classes_order,
    ...
    """
    Panel A: absolute volumes, inactive vs active, with BH-corrected
...
def plot_means_transparent_ax(ax, summary: pd.DataFrame, df: pd.DataFrame):
    """
    Panel B: mean log2(Active/Inactive) per class + points.
...
def plot_diff_matrix_lower_abs_ax(ax, pairs_df: pd.DataFrame,
    ...
    """
    Panel C: lower-triangular |Δmean| matrix with stars.
    """
    classes = summary["Class"].tolist()
...
def process_dataset(xlsx_path: Path, alpha: float, roi_label: str):
    ...
    print(f"[INFO] Processing ROI '{roi_label}': {xlsx_path}")
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate 1×3 figure (single ROI) for extracellular cavity volumes and export stats to Excel."
    )
    ap.add_argument(
        "--xlsx",
        default="volumen_5_portal_EC/gpcr_log2_active_inactive.xlsx",
        help="Path to ROI Excel file (gpcr_log2_active_inactive.xlsx).",
    )
    ap.add_argument(
        "--roi-label",
        default="Portal",
        help="Label for the ROI (for Excel and printed messages).",
    )
    ap.add_argument(
        "--outfig",
    ...
    ap.add_argument(
        "--excel-out",
        default="gpcr_intraacellular_stats_oneROI_BH.xlsx",
    ...
    # Process the single ROI
    roi_df, roi_summary, roi_pairs, roi_classes, roi_state = process_dataset(
        Path(args.xlsx), args.alpha, roi_label=args.roi_label
    )
    ...
    plot_active_inactive_absolute_ax(axA, roi_df, roi_classes, roi_state)
    plot_means_transparent_ax(axB, roi_summary, roi_df)
    imC = plot_diff_matrix_lower_abs_ax(axC, roi_pairs, roi_summary,
                                        alpha=args.alpha)
    ...
    plt.close(fig)
    print(f"[OK] Single-ROI figure written to {args.outfig}")

    # ==========================
    # Excel with statistics
    # ==========================
    state_all = roi_state.copy()
    class_all = roi_pairs.copy()
    summary_all = roi_summary.copy()
