...
"""
Compact two-region GPCR volume figure for OPV and hydrophobic slab by default.

This version replaces the right-column post-hoc heatmaps with compact letter
labels above the log2FC panels. The resulting layout is 2×2:

    first input region:  absolute volume      log2(active/inactive) + post-hoc letters
    second input region: absolute volume      log2(active/inactive) + post-hoc letters

Region names used in plot titles and Excel outputs are inferred automatically
from the parent folder names of the input Excel files, for example:
    volumen_7_OPV              -> Orthosteric pocket
    volumen_1_hydrophobic_slab -> Hydrophobic slab
    volumen_5_portal_EC        -> EC portal
    volumen_2_ec_vestibule     -> EC vestibule
    volumen_3_cleft_volume     -> TM5–TM7 cleft
    intracellular / IC folders  -> Intracellular cavity

Compact letter display (CLD) interpretation:
...
Expected input: two Excel files containing at least the canonical columns or
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate compact 2×2 two-region GPCR volume figure with post-hoc letters instead of heatmaps."
    )
    ap.add_argument(
        "--roi1-xlsx",
        default="volumen_7_OPV/gpcr_log2_active_inactive.xlsx",
        help="Path to the first region Excel. The region name is inferred from the parent folder unless --roi1-label is provided.",
    )
    ap.add_argument(
        "--roi1-label",
        default=None,
        help="Optional manual label for the first region. By default, it is inferred from the input folder name.",
    )
    ap.add_argument(
        "--roi2-xlsx",
        default="volumen_1_hydrophobic_slab/gpcr_log2_active_inactive.xlsx",
        help="Path to the second region Excel. The region name is inferred from the parent folder unless --roi2-label is provided.",
    )
    ap.add_argument(
        "--roi2-label",
        default=None,
        help="Optional manual label for the second region. By default, it is inferred from the input folder name.",
    )
    ap.add_argument(
        "--outfig",
        default="figure_OPV_hydrophobic_slab_2x2_letters.png",
        help="Output PNG file.",
    )
    ap.add_argument(
        "--outpdf",
        default="figure_OPV_hydrophobic_slab_2x2_letters.pdf",
        help="Output PDF vector file.",
    )
    ap.add_argument(
        "--excel-out",
        default="gpcr_OPV_hydrophobic_slab_stats_BH_2x2_letters.xlsx",
    ...
    roi1_label = args.roi1_label or infer_roi_label_from_path(args.roi1_xlsx)
    roi2_label = args.roi2_label or infer_roi_label_from_path(args.roi2_xlsx)

    print(f"[INFO] Region label inferred for first input:  {roi1_label}")
    print(f"[INFO] Region label inferred for second input: {roi2_label}")

    roi1 = process_dataset(Path(args.roi1_xlsx), args.alpha, roi_label=roi1_label)
    roi2 = process_dataset(Path(args.roi2_xlsx), args.alpha, roi_label=roi2_label)

    datasets = [roi1, roi2]
    roi_titles = [roi1_label, roi2_label]
    panel_labels = [("A", "B"), ("C", "D")]

    fig, axes = plt.subplots(
        2, 2,
        figsize=(args.fig_width, args.fig_height),
        gridspec_kw={"width_ratios": [1.12, 1.0], "wspace": 0.24, "hspace": 0.48},
    )
    ...
    fig.legend(handles=handles, loc="upper center", ncol=2, frameon=False,
               bbox_to_anchor=(0.52, 1.02), handlelength=1.2, columnspacing=1.3)
