...
ionic_lock_R3.50_E/D6.30, alt_lock_D/E3.49_K/R6.30, network_E/D3.39_H/K/R6.40, dry_D/E3.49_R/K3.50,
...
def compute_distances(path, chain, bw_window, policy):
    ...
    # Present flags
    ionic_lock_present = 1 if (filter_by_resname(c_350, {"ARG"}) and filter_by_resname(c_630, {"GLU","ASP"})) else 0

    row={
        # Locks (canónico y alternativo) + network + DRY
        "ionic_lock_R3.50_E/D6.30": float("nan") if (not c_350 or not c_630) else
            min_heavy_over_candidates_flexible(
                {"ASP": filter_by_resname(c_630, {"ASP"}), "GLU": filter_by_resname(c_630, {"GLU"})},
                {"ARG": filter_by_resname(c_350, {"ARG"})}
            )[0],
...
def main():
    ...
    fieldnames=[
        ...
        "ionic_lock_R3.50_E/D6.30","ionic_lock_present",
        "alt_lock_min_dist","alt_lock_present","alt_lock_used_3.49","alt_lock_used_6.30","alt_lock_used_bw_3.49","alt_lock_used_bw_6.30",
        "network_min_dist","network_present","network_used_3.39","network_used_6.40","network_used_bw_3.39","network_used_bw_6.40",
        "dry_min_dist","dry_used_3.49","dry_used_3.50","dry_used_bw_3.49","dry_used_bw_3.50",
        ...
        if args.workers > 1:
            ...
            with Pool(processes=args.workers) as pool:
                for res in pool.imap_unordered(_worker, todo, chunksize=chunksize):
                    ...
                    if tag=="OK":
                        row={"file":base,"chain_used":chain_used,"status":"OK","skip_reason":""}
                        row.update(payload); wr.writerow(row); fo.flush(); processed+=1
                        print(f"[OK] {base} | chain={chain_used}")
                    elif tag=="SKIP":
                        row={"file":base,"chain_used":chain_used,"status":"SKIP","skip_reason":str(payload)}
                        for k in fieldnames:
