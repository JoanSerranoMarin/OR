...
"""
Ionic / alternative / DRY locks en GPCRs usando:

- Secuencia canónica de GPCRdb (residues/extended)
- Búsqueda local de péptidos 2W+1 (por defecto 5 AA) centrados en BW 3.x / 6.x
- Ventana de ±N posiciones BW (por defecto 2) para elegir el residuo
  cargado real alrededor de la posición de referencia.

Estrategia:
-----------
Para cada PDB y cada motivo (ionic, alt, DRY):

1) Para cada lado del motivo (p.ej. helix 3, centro 50; helix 6, centro 30):
   - Tomamos todos los BW de esa hélice en el rango [pos - bw_window, pos + bw_window].
   - Para cada BW:
       * Buscamos su índice canónico en residues/extended.
       * Construimos un péptido 2*flank_5mer+1 centrado en ese residuo.
       * Buscamos ese péptido en la secuencia PDB (permitiendo <= MAX_MISMATCH).
       * Si se encuentra, ese residuo PDB es un "candidato BW".

2) Para cada motivo:
   - Filtramos candidatos por identidad (ARG/LYS/ASP/GLU) y por presencia de
     TODOS los átomos de cadena lateral requeridos.
   - Calculamos la distancia mínima N–O entre todas las parejas candidato_izq
     y candidato_der; CA–CA se calcula con la misma pareja.
   - Guardamos la pareja usada y sus BW.

3) El 5-mer sólo sirve para garantizar que numeramos fragmentos que
   pertenecen al receptor, no péptidos exógenos.

También:
- Clasificación de estado:
   Ago → Active
   Antagonist / inverse agonist → Inactive
   resto → Undefined
- Estadística (igual que antes): medias, log2FC, Welch, BH-FDR.
